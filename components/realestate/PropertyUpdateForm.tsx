...
const PropertyUpdateForm = ({ propertyId, property }: { propertyId?: string, property?: any }) => {
    const [isDetectingLocation, setIsDetectingLocation] = useState(false);
    const [existingImages, setExistingImages] = useState<string[]>(property?.images || []);
    const [existingPanoramicImages, setExistinPanoramicgImages] = useState<string[]>(property?.panoramicImages || []);
    const [imagePreviews, setImagePreviews] = useState<string[]>([]);
    const [panoramicPreviews, setPanoramicPreviews] = useState<string[]>([]);
    const [amenityInput, setAmenityInput] = useState("");
    const [amenities, setAmenities] = useState<string[]>([]);
    const [imagesToDelete, setImagesToDelete] = useState<string[]>([]);
    const [panoramicImagesToDelete,setpanoramicImagesToDelete] = useState<string[]>([]);
    ...
    const handleImageDelete = async (imageurl: string) => {
        try {
            ...
            const response = await fetch(
                `${process.env.NEXT_PUBLIC_API_ENDPOINT}/api/v1/properties/${property._id}/images`,
                {
                    method: "DELETE",
                    credentials: "include",
                  
                    body: JSON.stringify({ imagesToDelete }) // Send the specific image URL to delete
                }
            );
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 w-[70vw]">
        ...
        <div>
          {existingPanoramicImages.length > 0 && (
            <div>
                <h4 className="text-sm font-medium mb-2">Current Panoramic Images</h4>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
        ...
        render={({ field }) => (
            <FormItem>
            <FormLabel>Add Additional Panoramic Images for 360° view</FormLabel>
            <FormControl>
                <div
